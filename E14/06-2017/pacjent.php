...
<body>
    ...
    <section id="panel-prawy">
        ...
        <?php
            $id = $_POST['id'];
            $kwerenda2 = 'SELECT imie, nazwisko, choroby_przewlekle, uczulenia FROM pacjenci WHERE id =' . $id . ';';
            $wynik2 = $polaczenie->query($kwerenda2);
            $wynikTablica = $wynik2->fetch_assoc();
            
            echo '<p>Imię i nazwisko: ' . $wynikTablica['imie'] . ' ' . $wynikTablica['nazwisko'] . '</p>';
            echo '<p>Choroby przewlekłe: ' . $wynikTablica['choroby_przewlekle'] . '</p>';
            echo '<p>Uczulenia: ' . $wynikTablica['uczulenia'] . '</p>';
        ?>
    </section>
    <section id="stopka">
